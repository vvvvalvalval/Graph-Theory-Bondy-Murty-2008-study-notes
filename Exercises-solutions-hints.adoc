...
1. Given to edges `e1` and `e2` of G, exactly 1 autormorphism in `m` sends `e1` to `e2`.
2. To each spanning tree `T'` of `G/e` and each automorphism of `G`, there corresponds exactly one spanning tree `T(e')` of `G` with a marked edge `e'` with an automorphism sending `e'` to `e`.
